    ...
    squares.forEach(square => {
        ...
        if (move) {
            square.classList.add('possible-move');
            // Если это ход со взятием - добавляем красный стиль
    ...
    // console.log('Is own piece:', this.isOwnPiece(piece));
    ...
    // В остальных случаях снимаем выделение
...
//                         row: r, 
//                         col: c,
//                         isCapture: true // Помечаем ход как взятие
//                     });
//                 }
//                 break;
//             }
...
getKingMoves(row, col, piece) {
    ...
    for (let dir of directions) {
        ...
        if (newRow >= 0 && newRow < 8 && newCol >= 0 && newCol < 8) {
            ...
            // Клетка пустая или содержит фигуру противника
            if (!targetPiece || !this.isSameColor(piece, targetPiece)) {
                // Временно делаем ход
                ...
                // Проверяем, не атакована ли новая позиция короля
                const kingPos = { row: newRow, col: newCol };
                let isAttacked = false;
                
                // Проверяем только прямые атаки (без рекурсии)
    ...
    // Логика рокировки остаётся без изменений
    // ...
...
//             // Проверяем, что поле не находится под атакой
//             if (!this.isSquareAttacked(newRow, newCol, !isWhite)) {
//                 if (!targetPiece) {
//                     moves.push({ row: newRow, col: newCol });
//                 } else if (!this.isSameColor(piece, targetPiece)) {
//                     moves.push({ 
...
//     // Рокировка (остаётся без изменений)
//     const color = isWhite ? 'white' : 'black';
//     const backRank = isWhite ? 7 : 0;
...
//     filterLegalMoves(row, col, piece, moves) {
//     // Разрешаем все ходы, которые снимают шах (не только королем)
//     return moves.filter(move => {
//         const newPieces = this.copyBoard();
...
//     // Если король не под шахом, разрешаем все обычные ходы
//     if (!isInCheck) {
//         return moves.filter(move => {
//             const newPieces = this.copyBoard();
//             newPieces[move.row][move.col] = piece;
...
//     // Если король под шахом, разрешаем только ходы, которые снимают шах
//     return moves.filter(move => {
//         const newPieces = this.copyBoard();
//         newPieces[move.row][move.col] = piece;
//         newPieces[row][col] = '';
...
filterLegalMoves(row, col, piece, moves) {
    ...
    // Если король под шахом, разрешаем только ходы, которые снимают шах
    return moves.filter(move => {
        // Проверяем, блокирует ли ход атаку на короля
        if (this.doesMoveBlockCheck(row, col, move, piece, isWhite)) {
            return true;
        }
        
        // Также разрешаем ходы королем, которые уводят его из-под шаха
...
// Новая функция для проверки, блокирует ли ход атаку на короля
doesMoveBlockCheck(fromRow, fromCol, move, piece, isWhite) {
    if (piece.toLowerCase() === 'k') return false; // Король не может блокировать сам себя
    
    const kingPos = this.findKing(isWhite);
    if (!kingPos) return false;
    
    // Копируем доску и делаем ход
    const newPieces = this.copyBoard();
    newPieces[move.row][move.col] = piece;
...
isSquareAttacked(row, col, byWhite) {
    ...
    for (let dir of straightDirections) {
        ...
        while (r >= 0 && r < 8 && c >= 0 && c < 8) {
            ...
            if (piece) {
                ...
                break; // Прерываем, если нашли любую фигуру
            }
            r += dir.r;
            c += dir.c;
        }
    ...
    // Проверяем атаку короля (только на 1 клетку)
...
isKingInCheck(pieces, isWhite) {
    ...
    for (let r = 0; r < 8; r++) {
        for (let c = 0; c < 8; c++) {
            ...
            // Получаем возможные ходы фигуры (без проверки на шах)
